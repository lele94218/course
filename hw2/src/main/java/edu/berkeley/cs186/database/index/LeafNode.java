...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid)
        ...
        if (keys.size() < order) {
            ...
            // Case 2: overflow
            int left = order / 2, right = order - left + 1;
            ...
            if (index < left) {
                ...
                newKeys.add(index - left, key);
                newRids.add(index - left, rid);
            }
            LeafNode node = new LeafNode(metadata, newKeys, newRids, this.rightSibling);
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, int pageNum) {
        ...
        int rightPageNum = buf.getInt();
        Optional<Integer> rightSibling = (rightPageNum == -1) ? Optional.empty() : Optional.of(rightPageNum);
        int n = buf.getInt();
        for (int i = 0; i < n; ++ i) {
